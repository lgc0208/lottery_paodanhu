define(function (require, exports, module) {
  ...
  User.prototype.hitMove = function () {
    ...
    this.top =
      this.top < 0 ? 0 : this.top > this._maxTop ? this._maxTop : this.top;
    this.left =
      this.left < 0 ? 0 : this.left > this._maxLeft ? this._maxLeft : this.left;
  ...
  module.exports = {
    ...
    _bindUI: function () {
      ...
      $("#lucky-balls").on("click", "li", function (e) {
        ...
        console.log(options);
        if (!options) {
          that.addItem(id, name, company, prize, options);
          that.hit();
          el.remove();
        }
      });
      ...
      window.onload = function () {
        var tpl =
          '<div class="lottery-guide" id="guide-container">' +
    ...
    start: function () {
      ...
      if (
        currentLuckyNum >= firstPrizeNum + secondPrizeNum + thirdPrizeNum ||
        this.users.length == 0
      ) {
        alert("抽奖结束！谢谢参与~");
        return;
    ...
    stop: function () {
      ...
      // 中奖信息统计
      currentLuckyNum += 1;
      console.log("current num", currentLuckyNum);
      if (currentLuckyNum <= thirdPrizeNum) currentPrize = "third";
      else if (currentLuckyNum <= thirdPrizeNum + secondPrizeNum)
        currentPrize = "second";
      else currentPrize = "first";

      var data;
      switch (currentPrize) {
        case "first":
          data = _first_prize_data;
          break;
        case "second":
          data = _second_prize_data;
          break;
        case "third":
          data = _third_prize_data;
          break;
      }
      

      if (currentLuckyNum > firstPrizeNum + secondPrizeNum + thirdPrizeNum) {
        return;
      }

      var selectedUser = selectRandomUser(data);
      var lucky = this.users.find(function(user) {
    ...
    addItem: function (id, name, company, prize, options) {
      this.users.push(new User(id, name, company, prize, options));
    },
    ...
    moveLucky: function () {
      ...
      if (luckyUser) {
        ...
        console.log(luckyUser.prize);
        if (luckyUser.prize == "third")
          luckyUser.el.appendTo(
            "#lucky-balls-3"
          ); // 被选元素的结尾（仍位于内部）插入指定内容
        else if (luckyUser.prize == "second")
          luckyUser.el.appendTo("#lucky-balls-2");
        else luckyUser.el.appendTo("#lucky-balls");
    ...
    setHasLuckyData: function (item) {
      var arr = [];
      var hasLuckyData = _util.getStore("LUCKYDATA");
      if (hasLuckyData) {
      ...
      arr.push(item);
      _util.setStore("LUCKYDATA", arr); // 把开出来的名单存入localStorage
    },
  ...
  function selectRandomUser(data) {
  ...
  function hit(a, b) {
    ...
    var yStep =
      yOffset > 0
        ? Math.ceil((power * yOffset) / offset)
        : Math.floor((power * yOffset) / offset);
    var xStep =
      xOffset > 0
        ? Math.ceil((power * xOffset) / offset)
        : Math.floor((power * xOffset) / offset);
